import React, { Component } from "react";
import "../stylesheets/clients.scss";

export default class HelpWith extends Component {
  render() {
    return (
      <div className="read-clients">
        <div className="read-clients1">
          <li>Abuse</li>
          <li>ADD/ADHD</li>
          <li>Addictions</li>
          <li>AIDS/HIV</li>
          <li>Anger management</li>
          <li>Anxiety</li>
          <li>Asperger syndrome</li>
          <li>Bereavement</li>
          <li>Child related issues</li>
          <li>Chronic fatigue syndrome/ME</li>
          <li>Cultural issues</li>
          <li>Depression</li>
          <li>Eating disorders</li>
          <li>Identity issues</li>
          <li>Infertility</li>
          <li>Loss</li>
          <li>Men's issues</li>
          <li>Obsessions</li>
        </div>
        <div className="read-clients2">
          <li>Phobias</li>
          <li>Pregnancy related issues</li>
          <li>Redundancy</li>
          <li>Relationships</li>
          <li>Self esteem</li>
          <li>Self-harm</li>
          <li>Service veterans</li>
          <li>Sex-related issues</li>
          <li>Sexual identity</li>
          <li>Sexuality</li>
          <li>Spirituality</li>
          <li>Stress</li>
          <li>Trauma</li>
          <li>Women's issues</li>
          <li>Work related issues</li>
        </div>
      </div>
    );
  }
}
